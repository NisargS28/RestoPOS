...
async function seed() {
  ...
  try {
    ...
    console.log("✓ Permissions linked to SUPER_ADMIN");
    ...
    console.log("✓ Categories created for all branches");

    // Create Products for Branch 1
    const b1Categories = await prisma.category.findMany({
    ...
    console.log("✓ Products created for Branch 1");

    // Clone products to branch2 and branch3
    for (const branch of [branch2, branch3]) {
      const branchCats = await prisma.category.findMany({
    ...
    console.log("✓ Products cloned to other branches");

    const totalProducts = await prisma.product.count();

    console.log(
      `\n✅ Seed successful! Created 3 branches, ${categoryNames.length} categories per branch, ${totalProducts} total products, 4 roles, ${permissions.length} permissions.`
    );
    await prisma.$disconnect();
    process.exit(0);
  } catch (error) {
